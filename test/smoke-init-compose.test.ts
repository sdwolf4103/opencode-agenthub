...
test("setup mode settings compose into runtime config", async () => {
	...
	try {
		...
		const runtimeRootGitignorePath = path.join(workspace, ".opencode-agenthub", ".gitignore");
		...
		const runtimeRootGitignore = await readFile(runtimeRootGitignorePath, "utf8");

		expect(opencodeConfig.default_agent).toBe("auto");
		expect(opencodeConfig.agent.auto.permission["*"]).toBe("allow");
		expect(runtimeRootGitignore).toContain("Generated by opencode-agenthub");
		expect(runtimeRootGitignore).toContain("*");

		const secondResult = await composeWorkspace(workspace, "auto");
		const runtimeRootGitignoreAfterRecompose = await readFile(runtimeRootGitignorePath, "utf8");
		expect(secondResult.configRoot).toBe(result.configRoot);
		expect(runtimeRootGitignoreAfterRecompose).toBe(runtimeRootGitignore);
